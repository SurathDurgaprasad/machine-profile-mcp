    ...
    for name, query_func in targets:
        ...
        try:
            ...
            if hasattr(res, "collection_metadata"):
                ...
                status = "ok" if res["developer"].collection_metadata.status == "ok" and res["ai"].collection_metadata.status == "ok" else "partial"

            results.append((name, f"{round(duration_ms, 1)} ms", status.upper(), "PASSED"))
        except Exception as e:
            duration_ms = (time.perf_counter() - start_time) * 1000.0
            results.append((name, f"{round(duration_ms, 1)} ms", "ERROR", f"FAILED: {str(e)[:40]}"))

    print(tabulate(results, headers=["Tool Name", "Execution Latency", "Status Code", "Result State"]) if 'tabulate' in globals() else
