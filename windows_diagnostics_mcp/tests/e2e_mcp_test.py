...
async def run_e2e_test():
    ...
    if machine_profile_bin:
        ...
        server_params = StdioServerParameters(
            command=machine_profile_bin,
            args=[],
            env=env
        )
    else:
        logger.info("Console script not found in PATH. Falling back to python -m windows_diagnostics_mcp.server")
        server_params = StdioServerParameters(
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            expected_tools = {
                "system_summary", "machine_health", "developer_environment",
                "installed_tools", "ai_environment", "storage_summary",
                "running_processes", "network_summary"
            }
            ...
            logger.info(f"resources/list Success. Registered resources: {resource_uris}")

            expected_resources = {
                "windows://system", "windows://developer", "windows://ai", "windows://network"
            }
            assert expected_resources.issubset(resource_uris), f"Missing expected resources: {expected_resources - resource_uris}"
            ...
            assert "analyze_machine" in prompt_names, "Missing expected prompt 'analyze_machine'"
            ...
            for tool_name in expected_tools:
                ...
                assert len(tool_call.content) > 0, f"Tool {tool_name} returned empty content array"
                assert tool_call.content[0].type == "text", f"Tool {tool_name} returned non-text format"
                text_out = tool_call.content[0].text

                # Verify that it is valid JSON
                data = json.loads(text_out)
                assert isinstance(data, (dict, list)), f"Tool {tool_name} output is not a JSON object/array"

                # Verify that there are no Python tracebacks
                assert "traceback" not in text_out.lower(), f"Tool {tool_name} output contains a traceback"
                assert "tracebacktype" not in text_out.lower(), f"Tool {tool_name} output contains a traceback"
                assert "syntaxerror" not in text_out.lower(), f"Tool {tool_name} output contains a syntax error"
                assert "nameerror" not in text_out.lower(), f"Tool {tool_name} output contains a NameError"

                # Verify metadata exists on dict outputs
                if isinstance(data, dict):
                    if "collection_metadata" in data:
                        metadata = data["collection_metadata"]
                        assert "status" in metadata, f"Tool {tool_name} is missing metadata status"
                        assert "warnings" in metadata, f"Tool {tool_name} is missing metadata warnings"
                        logger.info(f"Tool {tool_name} SUCCESS. Status: {metadata['status']}, Latency: {metadata.get('duration_ms')} ms")
                    else:
                        logger.info(f"Tool {tool_name} SUCCESS (no direct metadata block, standard raw layout).")
                else:
            ...
            for resource_uri in expected_resources:
                ...
                assert len(resource_read.contents) > 0, f"Resource {resource_uri} returned empty contents"
                content_item = resource_read.contents[0]
                assert content_item.mimeType == "application/json", f"Resource {resource_uri} is not application/json"
                text_out = content_item.text

                # Verify that it is valid JSON
                data = json.loads(text_out)
                assert isinstance(data, dict), f"Resource {resource_uri} output is not a JSON object"

                # Check tracebacks
                assert "traceback" not in text_out.lower(), f"Resource {resource_uri} contains a traceback"
            ...
            assert len(prompt_get.messages) > 0, "Prompt analyze_machine returned empty message list"
            assert prompt_get.messages[0].role == "user", "Prompt message role is not 'user'"
            prompt_text = prompt_get.messages[0].content.text
            assert "Please call the appropriate diagnostic tools" in prompt_text, "Prompt instruction text mismatch"
            logger.info("Prompt analyze_machine SUCCESS.")

    logger.info("Shutting down E2E client. Subprocess terminated gracefully.")
    logger.info("SUCCESS: All E2E protocol tools, resources, and prompt handshakes validated successfully!")
