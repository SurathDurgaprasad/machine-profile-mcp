...
class DeveloperService:
    ...
    def _find_vscode(self) -> ToolInfoModel:
        ...
        if not executable:
            ...
            user_path = os.path.expandvars(r"%LocalAppData%\Programs\Microsoft VS Code\bin\code.cmd")
            if os.path.exists(user_path):
                executable = user_path
            else:
                # Check System install
                sys_path = os.path.expandvars(r"%ProgramFiles%\Microsoft VS Code\bin\code.cmd")
        ...
        try:
            ...
            return ToolInfoModel(installed=True, status="installed", version=version, path=executable)
        except FileNotFoundError:
            return ToolInfoModel(installed=False, status="not_detected")
        except TimeoutError:
            return ToolInfoModel(installed=True, status="unavailable", path=executable, error_message="VS Code version check timed out.")
        except Exception as e:
            return ToolInfoModel(installed=True, status="error", path=executable, error_message=str(e))

    def _check_tool(self, cmd_name: str, version_args: List[str], version_regex: str, strip_chars: str = "") -> ToolInfoModel:
        """
        ...
        try:
            ...
            if match:
                ...
                return ToolInfoModel(installed=True, status="installed", version=version, path=path)

            # Fallback if command runs but regex doesn't match
            fallback_version = output.strip().split("\n")[0]
            fallback_version = fallback_version[:40] if fallback_version else "Installed"
            return ToolInfoModel(installed=True, status="installed", version=fallback_version, path=path)
        except FileNotFoundError:
            return ToolInfoModel(installed=False, status="not_detected")
        except TimeoutError:
            return ToolInfoModel(installed=True, status="unavailable", path=path, error_message="Version check command timed out.")
        except Exception as e:
            return ToolInfoModel(installed=True, status="error", path=path, error_message=str(e))
    ...
    def get_developer_environment(self) -> DevEnvStatusModel:
        ...
        try:
            ...
            python_info = ToolInfoModel(installed=True, status="installed", version=python_version, path=python_path)
        except Exception as e:
            python_info = ToolInfoModel(installed=False, status="error", error_message=str(e))
            warnings.append(WarningItem(component="developer", code="PYTHON_QUERY_FAILED", message=f"Failed to query Python info: {e}"))
            status = "partial"
        ...
        for tool_name, tool_data in [
            ("git", git_info), ("node", node_info), ("docker", docker_info), ("java", java_info), ("vscode", vscode_info)
        ]:
